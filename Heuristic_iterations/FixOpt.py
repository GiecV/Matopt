...
class FixOpt:
    ...
    def compute_makespan(self): # Find the maximum makespan and the machine that has it
        ...
        self.solution = {key: round(value) for key, value in self.solution.items()}
    ...
    def solve_subproblem(self, N_prime, M_prime, fixed_jobs = {}): # Solve the subproblen, given the indeces of the jobs and machines
        ...
        s = Solver(execution_times = reduced_execution, setup_times = reduced_setup, time_limit=self.t_it,
                jobs_fixed_to_machine=converted_fixed_jobs)
    ...
    def select_machine(self, set, makespan): # Compute the probability of selecting a machine and select one at random
        ...
        for machine in set:
            p[machine] = max((makespan - self.compute_single_makespan(machine=machine))/makespan, 0.000001)
